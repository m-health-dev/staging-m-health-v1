...
export async function deleteInsurance(id: string) {
  try {
    console.log("Sending insurance/delete to BE:", id);
    ...
    if (res.status !== 200) {
      return {
        success: false,
        error: `Failed to sent insurance/delete data. Cause : ${res.status} - ${data.message}`,
      };
    }
    ...
    console.error("Sent insurance/delete Error:", error);
    return {
      success: false,
