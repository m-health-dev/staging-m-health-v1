...
const UpdateInsuranceForm = ({
  ...
  const form = useForm<z.infer<typeof InsuranceSchema>>({
    ...
    defaultValues: {
      ...
      en_description: insuranceData?.en_description || "",
      id_description: insuranceData?.id_description || "",
      category: insuranceData?.category || "",
      specialist: insuranceData?.specialist || [],
      highlight_image: insuranceData?.highlight_image || "",
      logo: insuranceData?.logo || "",
      agent_name: insuranceData?.agent_name || "",
      agent_number: insuranceData?.agent_number || "",
      agent_photo_url: insuranceData?.agent_photo_url || "",
    },
  });
  ...
  async function onSubmit(data: z.infer<typeof InsuranceSchema>) {
    setLoading(true);
    const res = await updateInsurance(data, id);
      ...
      <div className="my-10 bg-linear-to-b from-background via-background z-10 w-full py-5 flex justify-between items-center">
        <div>
          ...
          <h4 className="text-primary font-semibold ">
            {name ? name : "Update Insurance"}
          </h4>
          <p className="text-sm! text-muted-foreground">{insuranceData.slug}</p>
        </div>
        <VendorHotelDeleteCopyFunction
          id={id}
          deleteAction={deleteInsurance}
          name={insuranceData.name}
          locale={locale}
          resourceType="insurance"
          router={router}
          slug={`${baseUrl}/${locale}/insurance/${insuranceData.slug}`}
        />
      </div>
