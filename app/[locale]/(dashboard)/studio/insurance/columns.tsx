...
export const columns: ColumnDef<InsuranceType>[] = [
  ...
  },
  {
    ...
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Specialist" />
    ),
    cell: ({ row }) => {
      const specialist: string[] = row.getValue("specialist");
      return (
        <span className="text-wrap">
          {specialist.map((spec) => spec).join(", ")}
        </span>
      );
    },
  },
    ...
    cell: ({ row }) => {
      ...
      const handleDeleteInsurance = async () => {
        try {
          ...
          if (!res.error) {
            toast.success("Success to Delete Insurance", {
          ...
          setLoading(false);
          toast.warning("Failed to Delete Insurance", {
            description: `${err}`,
          });
        }
      };
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem asChild>
                <p
                  ...
                  onClick={() =>
                    router.push(`/${locale}/studio/insurance/update/${id}`)
                  }
                >
                  <PenSquare />
                  Update Data
          ...
          <Dialog open={openConfirm} onOpenChange={setOpenConfirm}>
            <DialogContent className="sm:max-w-md bg-white">
                ...
                <DialogTitle asChild>
                  <h6 className="text-red-500">
                    {locale === routing.defaultLocale
                      ? "Konfirmasi Penghapusan Asuransi"
                      : "Delete Insurance Confirmation"}
                  </h6>
                </DialogTitle>
                <p className="text-muted-foreground">
                  {locale === routing.defaultLocale
                    ? "Untuk menghapus asuransi ini, silahkan ketik nama asuransi:"
                    : "To delete this insurance, please type the insurance name:"}{" "}
                  <span
              ...
              <Input
                ...
                placeholder={
                  locale === routing.defaultLocale
                    ? "Tulis nama asuransi di sini"
                    : "Write insurance name here"
                }
              />
