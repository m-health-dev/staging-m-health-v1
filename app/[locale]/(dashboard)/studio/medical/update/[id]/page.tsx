...
const UpdateMedicalPage = async ({
  params,
}: {
  params: Promise<{ id: string }>;
}) => {
  const { id } = await params;
  const res = await getMedicalByID(id);

  if (res.error) {
    notFound();
  }
  return <UpdateMedicalForm id={id} medicalData={res.data.data} />;
};

export default UpdateMedicalPage;
