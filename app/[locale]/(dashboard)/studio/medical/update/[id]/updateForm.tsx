...
const UpdateMedicalForm = ({
  ...
  const [highlightPreview, setHighlightPreview] = useState<string | null>(
    medicalData.highlight_image,
  );
  const [referencePreview, setReferencePreview] = useState<string[]>(
    medicalData.reference_image,
  );
  ...
  const [name, setName] = useState(medicalData.id_title || "");
  ...
  const form = useForm<z.infer<typeof MedicalSchema>>({
    ...
    defaultValues: {
      en_title: medicalData?.en_title || "",
      id_title: medicalData?.id_title || "",
      en_tagline: medicalData?.en_tagline || "",
      id_tagline: medicalData?.id_tagline || "",
      highlight_image: medicalData?.highlight_image || "",
      reference_image: medicalData?.reference_image || [],
      duration_by_day: medicalData?.duration_by_day || 0,
      duration_by_night: medicalData?.duration_by_night || 0,
      spesific_gender: medicalData?.spesific_gender || "",
      en_medical_package_content: medicalData?.en_medical_package_content || "",
  ...
  async function handleBatchImageUpload(files: File[]) {
    ...
    formData.append("folder", "medical/contents");
  ...
  async function onSubmit(data: z.infer<typeof MedicalSchema>) {
    setLoading(true);
    const res = await updateMedical(data, id);

    if (res.success) {
      setLoading(false);
      toast.success(`${data.id_title} updated successfully!`);
      router.push(`/${locale}/studio/medical`);
    } else if (res.error) {
      setLoading(false);
    ...
    <ContainerWrap className="pb-20">
      <div className="my-10 bg-linear-to-b from-background via-background z-10 w-full py-5 flex justify-between items-center">
        ...
        <Studio1DeleteCopyFunction
          ...
          locale={locale}
          resourceLabel="Medical"
          router={router}
          slug={`${baseUrl}/${locale}/medical/${medicalData.slug}`}
        />
      </div>
