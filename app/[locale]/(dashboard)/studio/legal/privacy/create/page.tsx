...
import { postPrivacyPolicy } from "@/lib/legal/post-patch-legal";
const AddPackage = () => {
  ...
  const [name, setName] = useState("Kebijakan Privasi");
  ...
  async function onSubmit(data: z.infer<typeof LegalSchema>) {
    setLoading(true);
    const res = await postPrivacyPolicy(data);

    if (res.success) {
      setLoading(false);
      toast.success(`${data.en_title} renewed successfully!`);
      router.push(`/${locale}/studio/legal/privacy`);
    } else if (res.error) {
      setLoading(false);
