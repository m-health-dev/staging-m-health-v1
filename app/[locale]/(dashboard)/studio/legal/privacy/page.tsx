import React from "react";
import { columns } from "./columns";

import Link from "next/link";
import { ChevronDown, ChevronRight, Database, Plus } from "lucide-react";
import { Button } from "@/components/ui/button";
import { getLocale } from "next-intl/server";
import { routing } from "@/i18n/routing";

import ContainerWrap from "@/components/utility/ContainerWrap";

import { Studio1DataTable } from "@/components/package-wellness-medical/studio-1-data-table";

import { getAllPrivacyPolicy } from "@/lib/legal/get-legal";

const PrivacyStudioPage = async ({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}) => {
  const params = await searchParams;
  const page = Number(params.page ?? 1);
  const per_page = Number(params.per_page ?? 10);

  const { data, meta, links, total } = await getAllPrivacyPolicy(
    page,
    per_page,
  ); // nanti page bisa dynamic

  const locale = await getLocale();

  return (
    <ContainerWrap className="pb-[20vh]">
      <div className="my-10 flex items-center justify-between gap-5 sticky top-0 bg-linear-to-b from-background via-background z-20 py-5 w-full">
        <div className="flex flex-col w-full">
          <h4 className="text-primary font-semibold">Privacy Policy Data</h4>
        </div>
        <Link href={`/${locale}/studio/legal/privacy/create`}>
          <Button className="rounded-2xl flex lg:w-fit w-full">
            <Plus /> <p className="lg:block hidden">Renew Privacy Policy</p>
          </Button>
        </Link>
      </div>
      <p className="text-muted-foreground mb-5">
        Selalu periksa setiap melakukan pembaruan Kebijakan Privasi. Anda hanya
        dapat membuat/ menambahkan dan tidak dapat melakukan pembaruan/
        perubahan lagi setelah data Kebijakan Privasi diterbitkan. Untuk
        memperbarui anda harus membuat Kebijakan Privasi yang baru.
      </p>
      <div className="summary bg-white p-4 rounded-2xl border mb-4 flex flex-wrap gap-4 items-center">
        <p className="text-sm! text-muted-foreground inline-flex gap-2 items-center bg-accent px-3 py-1 rounded-xl">
          <Database className="size-4" />
          <span>
            {locale === routing.defaultLocale
              ? "Ringkasan Data"
              : "Data Summary"}
          </span>
        </p>
        <p className="font-light text-sm! text-muted-foreground">
          <ChevronRight className="size-4 lg:flex hidden" />
          <ChevronDown className="size-4 lg:hidden flex" />
        </p>
        <div className="flex flex-wrap gap-4 items-center">
          <p className=" bg-teal-300 rounded-xl px-3 py-1 text-sm! w-fit">
            {total} Privacy Policy Created
          </p>
        </div>
      </div>
      <Studio1DataTable
        columns={columns}
        data={data}
        meta={meta}
        links={links}
        type="legal"
        locale={locale}
      />
    </ContainerWrap>
  );
};

export default PrivacyStudioPage;
