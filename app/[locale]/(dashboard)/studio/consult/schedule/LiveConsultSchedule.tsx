...
interface LiveConsultScheduleProps {
  initialData: ConsultScheduleType[];
  meta: any;
  links: any;
  locale: string;
  page?: number;
...
const LiveConsultSchedule: React.FC<LiveConsultScheduleProps> = ({
  initialData,
  meta,
  links,
  locale,
  ...
  const [data, setData] = useState<ConsultScheduleType[]>(initialData);
      ...
      intervalRef.current = setInterval(async () => {
        try {
          ...
          const res = await fetch(
            `/api/live/consultation?page=${page}&per_page=${per_page}`,
          );
          if (res.ok) {
  ...
  return (
    <>
      ...
      <Studio1DataTable<ConsultScheduleType, unknown>
        columns={columns}
        data={data}
        meta={meta}
        links={links}
...
export default LiveConsultSchedule;
